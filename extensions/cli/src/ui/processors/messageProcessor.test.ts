  ...
  describe("processHistoryToMessageRows", () => {
    it("should render user messages with correct bullet and margin formatting", () => {
      const chatHistory = [{
        message: {
          role: "user" as const,
          content: "This is a long message that should be split across multiple rows to test the formatting and ensure that only the first row has a bullet point and only the last row has margin.",
        },
        contextItems: [],
      }];
  ...
  it("should handle multiple separate user messages correctly", () => {
    const chatHistory = [
      ...
      },
      {
        message: {
          role: "user" as const,
          content: "Second user message that is long enough to potentially wrap across multiple lines in a narrow terminal",
        },
        contextItems: [],
      }
    ];
    ...
    userGroupStarts.forEach((startIndex) => {
      ...
      while (endIndex + 1 < messageRows.length && 
  ...
  it("should debug output for a very long message case", () => {
    ...
    messageRows.forEach((row, index) => {
      console.log(`Row ${index}:`, {
        ...
        text: row.segments.map(s => s.text).join('').slice(0, 60) + "...",
      });
    });
    ...
    const chatHistory = [
      {
        message: {
          role: "user" as const,
          content: "Can you help me write a function that takes a list of numbers and returns the sum? Please make sure to handle edge cases.",
        },
        contextItems: [],
      },
    ];
