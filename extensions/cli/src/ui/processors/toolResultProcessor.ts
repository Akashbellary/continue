  ...
  for (const line of lines) {
    ...
    if (checkboxMatch) {
      ...
      rows.push({
        type: "content",
        segments: [
          { text: indent, styling: {} }, // Original indent only
          {
            text: isCompleted ? "✓" : "○",
...
export function processToolResultIntoRows({
  ...
  const getSummary = () => {
    ...
    // Check if it was an error - show full multi-line errors
    if (content.startsWith("Error")) {
      return content; // Return full error content, will be handled by multi-line logic
    }
